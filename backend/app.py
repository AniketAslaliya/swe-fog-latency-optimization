...
def process_fog_task(current_time):
    ...
    base_latency = 30  # ms
    complexity_factor = task['complexity'] / 100  # 0.5ms to 20ms
    processing_latency = base_latency + complexity_factor
    
    # Store processing time in task for cleanup calculation
...
def process_cloud_task(current_time):
    ...
    base_latency = 120  # ms
    complexity_factor = task['complexity'] / 80  # Slightly less sensitive to complexity
    processing_latency = base_latency + complexity_factor
...
def update_config():
    ...
    try:
        ...
        print(f"✅ Config updated: {config_data['network']['fog_nodes']} fog nodes")
...
def start_simulation():
    ...
    try:
        ...
        with state_lock:
            ...
            num_devices = simulation_state.get('config', {}).get('network', {}).get('iot_devices', 10)
            if len(simulation_state.get('device_priorities', {})) != num_devices:
                simulation_state['device_priorities'] = {}
                for i in range(1, num_devices + 1):
                    device_id = f'device_{i}'
                    if i <= num_devices * 0.3:
...
def get_analytics():
    ...
    if base_failures > 0:
        ...
        for _ in range(base_failures):
            weights = [1, 1.2, 0.8, 1.1, 0.9][:num_fog_nodes]
...
def run_simulation_background(duration):
    ...
    try:
        ...
        while time.time() < end_time and simulation_state['running']:
            ...
            # Process fog tasks (HIGH priority) - only if queue has tasks
            if simulation_state['pending_fog_tasks']:
            ...
            with state_lock:
                ...
                for task_id, task in simulation_state['active_tasks'].items():
                    ...
                    processing_time = task.get('processing_time', 0.1)  # Default 100ms
            ...
            # Process cloud tasks (LOW/MODERATE priority) - only if queue has tasks
            if simulation_state['cloud_tasks']:
            ...
            # Update latency history periodically
            if len(fog_latencies) > 0 or len(cloud_latencies) > 0:
                ...
                with state_lock:
                    ...
                    simulation_state['latency_history']['timestamps'].append(f"{elapsed:.0f}s")
